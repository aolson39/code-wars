...
// in numerous damaged cars, broken limbs and several killed pets. Your mission is to write a safer version of their controller 
// software.
...
// Output
// A string where each character represents one second and indicates the position of the door (0 if fully closed and 5 fully 
// open). The door starts moving immediately, hence its position changes at the same second as the event.

// Example
// ..P...O..... as input should yield 001234321000 as output
// P -> 

//check if element is P or O if it is switch directions or pause
// if opening or closing and not paused add one or subtract one
...
function door(events) {
    ...
    for(let i = 0; i < events.length; i++){
        // If the garage door is pushed, swith the direction and if moving, pause
        if(events[i] === 'P'){
            // if moving, make it stop
            if(moving){
                moving = false
            }else{
                moving = true;
        ...
        // if obstacle detected switch direction
        if(events[i] === 'O'){
            directionUp = !directionUp
        }
        // if moving add or substract position
        // if in full open or close position add to result and set moving to false
        if(moving){
            if(directionUp){
                position++
                result += position
