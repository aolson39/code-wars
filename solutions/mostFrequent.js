...
function topThreeWords(text) {
        ...
        const sorted = new Map([...map1].sort((a, b) => b[1] - a[1]));
        const result = Array.from(sorted.keys())
        let answer = []
        for(let i = 0; i < result.length; i++){
          if(result[i] !== null && answer.length <3 ){
            answer[i] = result[i]
          }
        }
        console.log(answer);
...
function isCharacterOnlyLetter(char) {
        return (/[a-zA-Z]/).test(char)
}
